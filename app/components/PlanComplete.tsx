...
export default function PlanComplete({ onSelect, selectedPaths }) {
    ...
    return (
        <Transition show={true} appear={true}>
            <div 
                id="plan_complete" 
                className={`relative p-4 h-full flex gap-20 flex-col justify-center items-center mt-24 transition duration-500 ease-in data-[closed]:opacity-0`}
                style={{ display: selectedOption ? 'none' : 'flex' }}
            >
                <Link
                    to={"plan"}
                    className={`relative flex flex-col h-24 w-60 hover:bg-zinc-950 justify-end items-center transition-opacity duration-500 ${selectedOption ? 'opacity-0' : 'opacity-100'}`}
